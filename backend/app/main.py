...
def root():
    return {
        ...
        "features": [
            ...
            "wall migration",
            "morning briefing",
            "Today / Week / Month / Combined engines",
            "Databento-ready futures data",
            "futures leadership and divergence",
            "distance-filtered actionable levels",
        ],
        "docs": "/docs",
    }
...
def roadmap(
    ...
    roadmap_data = get_decision_roadmap(symbol.upper(), timeframe)
    data = roadmap_data.model_dump()
    official = latest_structure(symbol.upper())
    if official:
        data["official_prior_day"] = official
    return data


@app.get("/api/futures-leadership/{symbol}")
def futures_endpoint(
    symbol: str,
    interval: str = "5m",
    _: dict = Depends(require_owner),
):
    return futures_leadership(symbol.upper(), interval=interval)
