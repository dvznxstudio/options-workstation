...
def enrich_chain(chain: pd.DataFrame, spot: float) -> pd.DataFrame:
    df = chain.copy()
    for c in ["strike","bid","ask","last","volume","open_interest","implied_volatility","delta","gamma"]:
        if c not in df.columns:
            df[c] = 0.0
        df[c] = pd.to_numeric(df[c], errors="coerce").fillna(0.0)
    df = df[(df["strike"] > 0) & (df["open_interest"] >= 0)].copy()
    deltas, gammas = [], []
    for row in df.itertuples(index=False):
        if float(row.gamma) > 0 and abs(float(row.delta)) <= 1.0:
            deltas.append(float(row.delta)); gammas.append(float(row.gamma))
        else:
            iv = float(row.implied_volatility)
            if iv > 3: iv /= 100.0
            d, g = greeks(spot, float(row.strike), years_to_expiration(str(row.expiration)),
                          max(iv, 0.0001), str(row.option_type))
...
def expected_move(chain: pd.DataFrame, spot: float) -> dict:
    ...
    exp = exps[0]
    sub = chain[chain.expiration==exp].copy()
...
def transitions(strikes: pd.DataFrame, spot: float):
    ordered = strikes.sort_values("strike")
    signs = np.sign(ordered.net_gex.to_numpy())
    vals=[]
    for i in range(1,len(ordered)):
        if signs[i] != signs[i-1] and signs[i] != 0:
            vals.append(float(ordered.iloc[i].strike))
...
def compute_structure(chain: pd.DataFrame, spot: float) -> dict:
    ...
    return {
      "strikes":s,"net_gex":float(s.net_gex.sum()),"net_dex":float(s.net_dex.sum()),
